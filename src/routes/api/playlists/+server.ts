...
export const GET = apiRoute('/api/playlists', 'GET', {
...
export const POST = apiRoute('/api/playlists', 'POST', {
	...
	try {
		...
		const playlist = await playlistService.create(locals.session.user.id, name.trim(), description?.trim());
